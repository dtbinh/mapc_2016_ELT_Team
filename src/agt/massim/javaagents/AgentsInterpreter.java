...
public class AgentsInterpreter implements Interpreter,AgentListener,EnvironmentListener {
	...
	public AgentsInterpreter() {
		...
		try {
			parseConfig("javaagentsconfig.xml");
	...
	private void parseConfig(String filename) throws ParseException {
		...
		if ( str != null && str.equals("") == false ) {
			if ( str.equals("yes") || str.equals("true") ) {
				...
				throw new ParseException("invalid value \"" + str + "\" for verbose-attribute of <entity>");
			}
		}
	...
	public StepResult step() {
		...
		for ( Agent ag : agents.values() ) {
			...
			// if an exception occurs it is printed. because this is expected not to happen
			// it qualifies as a severe issue when it does... thus execution stops for a second
			catch (Exception e) {
				e.printStackTrace();
				try {
					Thread.sleep(1000);
	...
	@Override
	public EnvironmentInterfaceStandard getEnvironment() {
		return Agent.getEnvironmentInterface();
	}
